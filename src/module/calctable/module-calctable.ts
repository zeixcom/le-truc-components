 ...
 **/
export default defineComponent("module-calctable", ({ first, host, on, watch }) => {
  ...
  reconcile(container, template, list, (element, item, key) => {
    ...
    const priceInput =
      element.querySelector<HTMLInputElement>("input.price-per-unit");
  ...
  on(container, "change", (e) => {
    ...
    const priceInput =
      entryRow.querySelector<HTMLInputElement>("input.price-per-unit");
