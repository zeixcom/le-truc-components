...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Text field to enter the name; updates <code>host.name</code> on each <code>input</code> event, falling back to the initial value when empty</td>
    </tr>
    <tr>
      <td><code>first('output')</code></td>
      <td><code>HTMLOutputElement</code></td>
      <td><strong>required</strong></td>
      <td>Displays the current name; its initial text content seeds the <code>name</code> property</td>
    </tr>
  </tbody>
</table>
